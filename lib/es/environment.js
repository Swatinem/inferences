...
DeclarativeEnvironment.prototype.CreateMutableBinding = function DeclarativeEnvironment_CreateMutableBinding(runtime, N, D) {
	...
	// TODO: non-deletable
	var cowed = runtime.cow(this.bindings);
	cowed[1].set(N, runtime.undefined); // bindings
	return cowed[0]; // runtime
};
// 10.2.1.1.3
DeclarativeEnvironment.prototype.SetMutableBinding = function DeclarativeEnvironment_SetMutableBinding(runtime, N, V, S) {
	// assert(this.HasBinding(N));
	// TODO: immutable binding?
	var cowed = runtime.cow(this.bindings);
	cowed[1].set(N, V); // bindings
	return cowed[0]; // runtime
};
// 10.2.1.1.4
...
// 10.2.1.1.5
DeclarativeEnvironment.prototype.DeleteBinding = function DeclarativeEnvironment_DeleteBinding(runtime, N) {
	if (!this.HasBinding(N)) {
		return [runtime, true];
	}
	// TODO: non-deletable
	var cowed = runtime.cow(this.bindings);
	cowed[1].delete(N); // bindings
	return [cowed[0], true]; // runtime
};
// 10.2.1.1.6
...
ObjectEnvironment.prototype.CreateMutableBinding = function ObjectEnvironment_CreateMutableBinding(runtime, N, D) {
	...
	return this.object.DefineOwnProperty(runtime, N, desc, true);
};
// 10.2.1.2.3
ObjectEnvironment.prototype.SetMutableBinding = function ObjectEnvironment_SetMutableBinding(runtime, N, V, S) {
	return this.object.Put(runtime, N, V, S);
};
// 10.2.1.2.4
...
// 10.2.1.2.5
ObjectEnvironment.prototype.DeleteBinding = function ObjectEnvironment_DeleteBinding(runtime, N) {
		return this.object.Delete(runtime, N);
};
// 10.2.1.2.6
...
// toString methods
ExecutionContext.prototype.toString = function ExecutionContext_toString() {
	return this.LexicalEnvironment.toString();
};
ObjectEnvironment.prototype.toString = function ObjectEnvironment_toString() {
	return this.object.toString();
};
LexicalEnvironment.prototype.toString = function LexicalEnvironment_toString() {
	return this.record.toString() /* + outer… */;
};
