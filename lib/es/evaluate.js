...
f
		...
		case 'MemberExpression':
			...
			*/
			...
			ast.object.Value.push(baseValue);

			var ref = new runtime.MultiReference();
			baseValue.forEach(function (base) {
				if (!ast.computed) {
					ref.push(new runtime.Reference(base, ast.property.name, strict));
				} else {
		...
		case 'UnaryExpression':
			switch (ast.operator) {
				...
				case 'delete':
					...
					if (ref.IsPropertyReference()) {
						return new runtime.BooleanValue(runtime.ToObject(ref.base).Delete(ref.name, ref.strict));
					} else {
						// TODO: 5.a strict: throw SyntaxError
						var bindings = ref.base;
						return new runtime.BooleanValue(bindings.DeleteBinding(ref.name));
					}
		...
		case 'ConditionalExpression':
			...
			// both true and false:
				...
				var val = new runtime.MultiValue();
				val.push(runtime.GetValue(recurse(ast.consequent)));
				val.push(runtime.GetValue(recurse(ast.alternate)));
				return val;
			}
		...
		case 'ExpressionStatement':
			var exprRef = recurse(ast.expression);
			return runtime.GetValue(exprRef);
		...
		case 'ReturnStatement':
			...
			if (!ast.argument)
				return context.return.push(runtime.undefined);
			// else
			var exprRef = recurse(ast.argument);
			return context.return.push(runtime.GetValue(exprRef));
		...
		case 'FunctionExpression':
			if (ast.id) {
				...
				var closure = new runtime.FunctionValue(ast, funcEnv, false);
				envRec.InitializeImmutableBinding(ast.id.name, closure);
				return closure;
			} else {
				return new runtime.FunctionValue(ast, context.LexicalEnvironment, false); // TODO: strict mode?
			}
