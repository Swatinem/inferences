...
ObjectValue.prototype.Put = function ObjectValue_Put(runtime, P, V, Throw) {
	...
	if (runtime.IsDataDescriptor(ownDesc)) {
		...
		this.DefineOwnProperty(runtime, P, valueDesc, Throw);
		return runtime;
	}
	var desc = this.GetProperty(P);
	if (runtime.IsAccessorDescriptor(desc)) {
		...
		this.DefineOwnProperty(runtime, P, newDesc, Throw);
		return runtime;
	}
};
// 8.12.6
...
ObjectValue.prototype.Delete = function ObjectValue_Delete(P, Throw) {
	...
	if (desc.Configurable) {
		delete this.properties.delete(P);
		return true;
	}
	// TODO: TypeError
	return false;
...
function FunctionValue(runtime, ast, Scope, strict) {
	...
	F.DefineOwnProperty('length', new runtime.PropertyDescriptor({
		Value: len,
		Writable: false,
		Enumerable: false,
		Configurable: false
	...
	proto.DefineOwnProperty('constructor', new runtime.PropertyDescriptor({
		Value: F,
		Writable: true,
		Enumerable: false,
		Configurable: true
	}), false);
	F.DefineOwnProperty('prototype', new runtime.PropertyDescriptor({
		Value: proto,
		Writable: true,
		Enumerable: false,
		//Configurable: false
...
ObjectValue.prototype.toString = function ObjectValue_toString() {
	return '{' + this.properties.items().map(function (item) { return item[0] + ':…'; }).join(', ') + '}';
};
