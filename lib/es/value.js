...
FunctionValue.prototype.Call = function FunctionValue_Call(runtime, thisVal, argList) {
	// 10.4.3
	if (!thisVal)
		thisVal = runtime.globalObject;
	var localEnv = runtime.NewDeclarativeEnvironment(this.Scope);
	var funcCtx = new runtime.ExecutionContext(localEnv, localEnv, thisVal, runtime.globalObject);
	//funcCtx.func = this; // FIXME
	runtime.DeclarationBinding(this.Code, funcCtx, argList);
	//console.log(funcCtx.ThisBinding.properties);

	var oldContext = runtime.context;
	runtime.context = funcCtx;
	var result = runtime.inference(this.Code.body, runtime);
	//console.log(result, this.Code.body)

	var returnContext = result.output.get(result.cfg[1]).first().context;
	runtime.context = oldContext;
	//console.log('result', returnContext);
