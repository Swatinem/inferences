...
function inference(ast) {
	...
	var output = analyses(cfg, function (input, list, output) {
		...
		//console.log('output:', resultValue);

		if (!this.true || !resultValue)
			return input; // let the worklist worry about old output etc...

		if (output && output.first().equals(input))
			return {output: input, enqueue: false}; // no change -> algorithm ends
